...
/* 置1：置能低功耗tickless模式；置0：保持系統節拍（tick）中斷一直運行
...
/***************************************************************
             FreeRTOS與Hook函數有關的配置選項                                            
**************************************************************/
/* 置1：使用空閒Hook（Idle Hook類似於回呼函數）；置0：忽略空閒Hook
 * 
 * 空閒任務Hook是一個函數，這個函數由使用者來實現，
 * FreeRTOS規定了函數的名字和參數：void vApplicationIdleHook(void )，
 * 這個函數在每個空閒任務週期都會被調用
 * 對於已經刪除的RTOS任務，空閒任務可以釋放分配給它們的堆疊記憶體。
 * 因此必須保證空閒任務可以被CPU執行
 * 使用空閒Hook函數設置CPU進入省電模式是很常見的
 * 不可以調用會引起空閒任務阻塞的API函數
 */
#define configUSE_IDLE_HOOK						0      

/* 置1：使用時間片Hook（Tick Hook）；置0：忽略時間片Hook
 * 
 * 
 * 時間片Hook是一個函數，這個函數由使用者來實現，
...
/* 與函式configUSE_TRACE_FACILITY同時為1時會編譯下面3個函數
...
#define INCLUDE_xTaskGetCurrentTaskHandle           1   // 啟用一個可選函數（該函數被 Trace程式碼使用，預設該值為0 表示不用）
#endif
